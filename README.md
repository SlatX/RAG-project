...
A Retrieval-Augmented Generation (RAG) system engineered to ingest technical security advisories, vulnerability disclosures, and red team reports, enabling accurate, context-grounded AI search and question answering.

Instead of manually parsing lengthy technical reports, security analysts can query documents directly to extract indicators of compromise (IoCs), map attack chains, and evaluate vulnerability severity with cited sources.

---

## 🎯 Key Objectives

- **Targeted Document Ingestion:** Efficiently load, parse, and chunk structured and unstructured cybersecurity PDF reports.
- **High-Fidelity Embeddings:** Transform chunked report data into dense vector representations for semantic indexing.
- **Secure Vector Search:** Retrieve top-$k$ relevant contextual chunks via the Qdrant vector database.
- **Grounded Generation:** Enforce strict prompt-context augmentation to eliminate hallucination in security-critical workflows.
- **Adversarial Resilience:** Benchmark system defenses against indirect prompt injection and context exfiltration using Promptfoo.
...
- **Event-Driven Ingestion:** Inngest
- **Embeddings & LLM:** OpenAI API[cite: 18]
- **Security & Quality Evaluation:** Promptfoo[cite: 18]
